...
- **IQ Score**: la feature real es `src/utils/financialScore.js` (`calcFinancialScore`, 0-100, 5 factores de 20pts c/u: tasa de ahorro/presupuestos/deuda/metas/suscripciones) + la card de `Dashboard/index.jsx`. **Los factores NO coinciden con los 5 documentados en el brand book** (flujo de caja 30%/colchón 20%/deuda 20%/metas 15%/consistencia 15%) — es una decisión de PRODUCTO (cambiar pesos/factores reales), no de estilo, y quedó sin tocar a propósito. Igual con las 4 etiquetas de estado (`score.excellent/good/fair/critical`) vs. las 3 documentadas (Bien/Atención/Riesgo) — la app real usa 4 tiers, no se forzaron a 3. Sí se aplicó: nombre "IQ Score" (antes "Puntaje de salud financiera" / "Salud financiera"), ícono dedicado (`IconIQScore`, único que reusa el arco de 270°), y motion (`overshoot` + 900ms).
